...
DV$Condition <- relevel(DV$Condition, ref = 1)

ggplot(data = DV, aes(x = Condition, y = Gaze, colour = Condition)) + 
  geom_boxplot() + 
  guides(colour = FALSE)
...
r_neutral <- lme4::ranef(model.full)$Subject$ConditionNeutral
qqnorm(r_neutral)
qqline(r_neutral)

r_positive <- lme4::ranef(model.full)$Subject$ConditionPositive
qqnorm(r_positive)
qqline(r_positive)

# GLMM for binomial data - 1-factor with(repeated measures) with 3 levels 
RO <- read_csv("data_files/RO.csv", col_types = cols(Condition = col_factor(levels = c("Neutral", 
                                                                            "Positive", "Negative"))))
...
gamma_DV <- filter(DV, !is.na(RT))
model.gamma <- glmer(RT ~ Context * Sentence + (1 | Subject) + 
                       (1 | Item), data = gamma_DV, family = Gamma)

model.nongamma <- lmer(RT ~ Context * Sentence + (1 | Subject) + 
                       (1 | Item), data = gamma_DV)

qqnorm(residuals(model.gamma))
summary(model.gamma)
summary(model.nongamma)

fitdistrplus::descdist(DV[!is.na(DV$RT),]$RT)
hist(gamma_DV[!is.na(gamma_DV$RT),]$RT)
